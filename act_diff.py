...
def _pgd_blackbox(model_target,
                  ...
                  num_steps=args.num_steps,
                  step_size=args.step_size):
    out_target, act_list_target = model_target.forward_act(X)
    out_source, act_list_source = model_source.forward_act(X)
    for i in range(len(act_list_target)):
        diff = torch.subtract(act_list_target[i],act_list_source[i])
        print(diff)
        break
...
def eval_adv_test_blackbox(model_target, model_source, device, test_loader):
    ...
    for data, target in test_loader:
        ...
        # pgd attack
        X, y = Variable(data, requires_grad=True), Variable(target)
        _pgd_blackbox(model_target, model_source, X, y)
        break
    #     robust_err_total += err_robust
    #     natural_err_total += err_natural
    # print('natural_err_total: ', natural_err_total)
